...
// (ровно то, что "не пересказ по пунктам" и просило избежать). Итерация 3
// (текущая) — по CSV пользователя из отчёта по реальным ответам, без
// изменения формулировки: явно просит острые темы + настроение шуток +
// "не сглаживай тон", без прежнего "1-2 ярких детали, не больше".
const SUMMARY_PROMPT =
	"Сожми переписку в общее ощущение разговора, не построчный пересказ и не попытка упомянуть каждого участника по очереди, без пустых общих фраз. Опиши примерно острые темы, кто о них говорит и в каком настроении, передай настроение шуток. Не сглаживай тон. Только сама сводка, 2-4 предложения, без вступлений.";
